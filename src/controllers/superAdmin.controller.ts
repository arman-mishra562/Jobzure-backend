...
export const getUserPersonalDetails = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to fetch personal details', error: error instanceof Error ? error.message : error });
    }
};

// Update a user's personal details
export const updateUserPersonalDetails = async (req: Request, res: Response) => {
    try {
        const { userId } = req.params;
        const {
            full_name,
        ...
        const details = await prisma.personalDetails.update({
            ...
            data: {
                ...
                targetJobLocation: {
                    set: [],
                    connectOrCreate: (targetJobLocation as string[]).map((value: string) => ({ where: { value }, create: { value } })),
                },
                interestedRoles: {
                    set: [],
                    connectOrCreate: (interestedRoles as string[]).map((value: string) => ({ where: { value }, create: { value } })),
                },
                intrstdIndstries: {
                    set: [],
                    connectOrCreate: (intrstdIndstries as string[]).map((value: string) => ({ where: { value }, create: { value } })),
                },
            },
            include: {
                targetJobLocation: true,
        ...
        res.status(200).json({ message: 'Personal details updated', personalDetails: details });
    } catch (error) {
        console.error('Failed to update personal details:', error);
        res.status(500).json({ message: 'Failed to update personal details', error: error instanceof Error ? error.message : error });
    }
};

// Delete a user's personal details
export const deleteUserPersonalDetails = async (req: Request, res: Response) => {
    try {
        const { userId } = req.params;
        const details = await prisma.personalDetails.findUnique({ where: { userId: parseInt(userId) } });
        if (!details) {
            return res.status(404).json({ message: 'Personal details not found' });
        }
        await prisma.personalDetails.delete({ where: { userId: parseInt(userId) } });
        res.status(200).json({ message: 'Personal details deleted' });
    } catch (error) {
        console.error('Failed to delete personal details:', error);
        res.status(500).json({ message: 'Failed to delete personal details', error: error instanceof Error ? error.message : error });
    }
};
...
export const processAssignmentQueue = async () => {
    ...
    const admins = await prisma.admin.findMany({ include: { assignedUsers: true } });
    ...
    const queue = await prisma.userAssignmentQueue.findMany({ orderBy: { createdAt: 'asc' } });
    for (const admin of availableAdmins) {
        ...
        while (slots > 0 && queue.length > 0) {
            ...
            if (!nextUser) break;
            await prisma.user.update({ where: { id: nextUser.userId }, data: { assignedAdminId: admin.id } });
            await prisma.userAssignmentQueue.delete({ where: { userId: nextUser.userId } });
            slots--;
        }
    }
};
...
export const getAssignmentQueue = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to fetch assignment queue', error: error instanceof Error ? error.message : error });
    }
};
